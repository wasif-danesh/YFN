# Greater Melbourne application dataset

Prepared for the **Your Friendly Neighbourhood** application and approved for this project release. It includes all **361 spatial SA2s in ASGS 2021 GCCSA `2GMEL`**, with four indicator records each: **1,444 observations** and **2,166 annual population records**. Inputs are real official data, and the release metadata records `publication_ready=true`. Source dates, spatial coverage and known limitations remain part of every interpretation.
...
| Transport | 360 | 1 | Area-weighted raw morning access index over covered land; 354 available and 6 limited due to coverage below 50% |
| Selected public open space | 354 | 7 | Unioned selected inventory area / 2025 ERP; 354 available and 7 unavailable |
...
West Melbourne - Industrial and Royal Botanic Gardens Victoria have zero ERP throughout 2020–2025. They remain in `areas` and `population_history`, but `is_comparable = 0` and growth/per-resident values are unavailable. The other 359 areas have positive 2025 ERP and `is_comparable = 1` under the project's comparison-eligibility rule. This flag enables the application's raw-measure comparisons; it is not an overall neighbourhood score or a claim of personal suitability.
...
The project release uses this approved explicit filter:
...
- SQLite integrity, foreign keys, null handling, negative growth, source relationships and read-only runtime access are checked. All **17 tests pass** (one needs the saved raw source files, so it is skipped on a fresh clone such as GitHub CI), including the original spatial edge cases and a check that the ER diagram matches every table, foreign-key relationship and selected column.
...
Source checks and calculations are verified to this documented scope. The transport aggregation, 50% transport coverage threshold and selected open-space filter are approved for this project release. The application intentionally provides no overall neighbourhood score or external benchmark. It also does not claim field-verified current conditions: source dates, limited coverage and inventory limitations remain visible in the API and interface.

## Rebuild and development use

From the project root, use Python 3.12 with `pipeline/requirements-greater-melbourne.txt` installed in a virtual environment. This pins the spatial/workbook libraries and NumPy. Do not use Python `-O`: assertions intentionally stop invalid builds.
...
`curated/*.csv` is the normalized interchange format. Blank numeric cells mean null; identifiers must be imported as text. The FastAPI service reads `yfn.sqlite` in read-only mode and exposes the versioned contract in [`contracts/`](../../contracts/); the frontend consumes those JSON responses. Geometry and raw downloads need not be shipped with the API.
